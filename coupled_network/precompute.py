...
def create_environments(datasets, testing_set_size, filter_noise=True):
    ...
    Warning: The environments here are not the ones described in the paper (they are a superset).
             Here, we compute the expectation over all random seeds of the sign error for every
             possible pair of hyperparameter combinations. The resulting matrix is used in later
             code to produce the paper figures. This is done to allow the computation of the
             results in Section 6 and also those reported in the appendix.
